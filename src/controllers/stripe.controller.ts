...
export class StripeController {
  ...
  private async createStripeCheckoutSession(customerID: any): Promise<string> {
    ...
    await this.stripe.checkout.sessions
      .create({
        ...
        success_url:
          'http://localhost:3000/stripe/checkout/success?session_id={CHECKOUT_SESSION_ID}',
  ...
  // Map to `POST / members`
  @post('/members')
  async signupMember(@requestBody() payload: SignupPayload): Promise<any> {
    // Check if customer already exists
    this.stripe.customers
      ...
      .then(async (customerArray: any) => {
        if (customerArray.data.length > 0) {
          const customerID = customerArray.data[0]
            ? customerArray.data[0].id
            : null;
          console.debug(`Customer ${customerID} already exists`);
  ...
  private async createCustomerAndSubsription(
  ...
  private async createSubscription(customerID: any): Promise<any> {
    ...
    await this.stripe.subscriptionSchedules
      .create({
        customer: customerID,
        start_date: moment()
          .utc()
      ...
      .then(async (subscription: any) => {
        ...
        console.debug(
          `New subscription ${subscriptionID} customer ${customerID} created`,
        );
