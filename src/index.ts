...
export async function main(options: ApplicationConfig = {}) {
  ...
  const dbc = new DashboardController();
  dbc.redisGetMollieCustomers().then((custList: any) => {
    custList.forEach((custKey: string) => {
      dbc.redisGetMollieCustomer(custKey.replace(`${RedisUtil.mollieCustomerPrefix}:`, '')).then(async (custObj: any) => {
        debugCron(custObj);
        ...
        RedisUtil.redisGetAsync(`${RedisUtil.teamMemberPrefix}:${custObj.data.email.toLowerCase()}`).then((reply: any) => {
    ...
    dbc.redisGetDiscourseCustomers().then((discourseList: any) => {
      discourseList.forEach((custKey: string) => {
        dbc.redisGetDiscourseCustomer(custKey.replace(`${RedisUtil.discourseCustomerPrefix}:`, '')).then((custObj: any) => {
          debugCron(custObj);

          RedisUtil.redisGetAsync(`${RedisUtil.teamMemberPrefix}:${custObj.data.email}`).then((reply: any) => {
            debugCron(`Redis returend ${reply}`);
            if (reply == null) {
              // Store in Redis
              const redisMemberPayload = {
